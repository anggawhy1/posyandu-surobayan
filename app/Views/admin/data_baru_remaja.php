...
<div class="p-6 bg-gray-100 min-h-screen">
                    ...
                    <?php foreach ($remaja as $index => $row): ?>
                        <tr data-id="<?= $row['id'] ?>" class="bg-white border">
                            ...
                            <?php foreach (
                                ...
                                <td class="p-2 border text-center edit-remaja" contenteditable="true" data-field="<?= $field ?>">
                                    <?= esc($row[$field]) ?>
                                </td>
                            <?php endforeach; ?>
    ...
    <?php else : ?>
        <div class="mt-4 text-center">
            <p class="text-red-500">Tidak ada data remaja putri baru untuk dikonfirmasi.</p>
        </div>
    <?php endif; ?>
</div>
...
<script>
    ...
    document.getElementById("btnLanjutSimpan").addEventListener("click", function() {
        ...
        row.querySelectorAll(".edit-remaja").forEach(cell => {
            data[cell.dataset.field] = cell.innerText.trim();
        });

        fetch("/admin/data-baru-remaja/update", {
            method: "POST",
            headers: {
    ...
    document.getElementById("btnLanjutKonfirmasi").addEventListener("click", function() {
        fetch(`/admin/data-baru-remaja/konfirmasi/${idTerpilih}`, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                }
    ...
    document.getElementById("btnLanjutHapus").addEventListener("click", function() {
        fetch(`/admin/data-baru-remaja/hapus/${idTerpilih}`, {
            method: 'DELETE',
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
