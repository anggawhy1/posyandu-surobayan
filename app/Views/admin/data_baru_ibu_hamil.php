...
<div class="p-6 bg-gray-100 min-h-screen">
            ...
            <table class="table-auto w-full border-collapse border border-gray-300">
                <thead class="bg-gray-700 text-white">
                    <tr>
                        ...
                        <th class="p-2 border">NIK Ibu Hamil</th>
                        <th class="p-2 border">Nama Ibu Hamil</th>
                        <th class="p-2 border">NIK Suami</th>
                        <th class="p-2 border">Nama Suami</th>
                        <th class="p-2 border">Pekerjaan Ibu Hamil</th>
                        <th class="p-2 border">Pekerjaan Suami</th>
                        <th class="p-2 border">Tanggal Mulai Hamil</th>
                        <th class="p-2 border">Tanggal Perkiraan Lahir</th>
                        <th class="p-2 border">Usia Kehamilan</th>
                        <th class="p-2 border">Golongan Darah Ibu Hamil</th>
                        <th class="p-2 border">Golongan Darah Suami</th>
                        <th class="p-2 border">Kadar HB</th>
                        <th class="p-2 border">BB Sebelum Hamil</th>
                    ...
                    <?php foreach ($ibuHamil as $index => $row): ?>
                        <tr data-id="<?= $row['id'] ?>" class="bg-white border">
                            ...
                            <?php foreach (
                                ...
                                <td class="p-2 border text-center edit-ibu-hamil" contenteditable="true" data-field="<?= $field ?>">
                                    <?= esc($row[$field]) ?>
                                </td>
                            <?php endforeach; ?>
    ...
    <?php else : ?>
        <div class="mt-4 text-center">
            <p class="text-red-500">Tidak ada data ibu hamil baru untuk dikonfirmasi.</p>
        </div>
    <?php endif; ?>
</div>
...
<script>
    ...
    document.querySelectorAll(".edit-ibu-hamil").forEach(cell => {
    ...
    document.getElementById("btnLanjutSimpan").addEventListener("click", function() {
        ...
        row.querySelectorAll(".edit-ibu-hamil").forEach(cell => {
            data[cell.dataset.field] = cell.innerText.trim();
        });

        fetch("/admin/data-baru-ibu-hamil/update", {
            method: "POST",
            headers: {
    ...
    document.getElementById("btnLanjutKonfirmasi").addEventListener("click", function() {
        fetch(`/admin/data-baru-ibu-hamil/konfirmasi/${idTerpilih}`, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                }
    ...
    document.getElementById("btnLanjutHapus").addEventListener("click", function() {
        fetch(`/admin/data-baru-ibu-hamil/hapus/${idTerpilih}`, {
            method: 'DELETE',
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
